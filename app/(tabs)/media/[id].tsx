    ...
    <View style={{ flex: 1, backgroundColor: theme.background }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Card variant="elevated" padding="large" style={{ marginBottom: 16 }}>
          ...
          <View style={localStyles.mediaPreview}>
            {media.type === 'image' && media.thumbnail ? (
              ...
              <View style={[localStyles.previewPlaceholder, { backgroundColor: getTypeColor() }]}>
                <Ionicons name={media.type === 'video' ? 'videocam' : media.type === 'document' ? 'document' : media.type === 'audio' ? 'musical-notes' : 'image'} size={48} color="#ffffff" />
              </View>
            )}
          </View>
          <View style={localStyles.row}>
            <Ionicons name={media.type === 'video' ? 'videocam' : media.type === 'document' ? 'document' : media.type === 'audio' ? 'musical-notes' : 'image'} size={18} color={getTypeColor()} style={{ marginRight: 6 }} />
          ...
          {media.size && (
            <View style={localStyles.row}>
              <Ionicons name="document" size={18} color={theme.primary} style={{ marginRight: 6 }} />
